...
export default class Store {
    ...
    async updateAvatar(file:FormData) {
        try {
            axios.post(`http://localhost:8000/upload${this.user.id}`, file)
        } catch (e) {
            console.log(e);
        }finally{
            this.setWaiting(false);
    ...
    async addWrite(to:string,date:string,time:string,contact:string){
        try{
            this.setWaiting(true);
            const name=this.user.lastName+' '+this.user.firstName;
            const response =await AuthService.addWrite(this.user.email,name,to,contact,date,time)
        }catch(e){
            console.log(e);
        }finally{
            this.setWaiting(false);
